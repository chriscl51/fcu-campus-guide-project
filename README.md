...
- **地圖**：沒有使用 Google Maps 或任何地圖 API/金鑰。校園地圖是把 `scripts/` 裡真實
  ...
  （西門/東門/北門/南門）也畫成紅色標記，跟官方地圖的紅字門標一致。因為底層座標
...
| 南門座標（估算） | `map/fcu map.jpg`（你提供的平面圖）＋ 3 個已知校門實測座標做仿射換算 | 見下方說明 |
...
**校門**：西門/大門口、東門、北門有實測座標。**南門在原始測繪資料裡完全找不到座標
點**，所以按照你的指示改用平面圖估算補上（`scripts/build_gates.py`）：取平面圖上 3
個已知校門的像素位置對應真實 GPS 座標，用最小平方法（`numpy.linalg.lstsq`）解出
像素→經緯度的仿射轉換，再套用到南門在平面圖上的像素位置。南門資料多了
`"approx": true` 標記，地圖上用紅色虛線圈＋滑鼠提示註明「估算位置，非實測」，不會
假裝是精確測繪資料。如果你之後用 JOSM 實測南門座標，把 `build_gates.py` 的
`GATES` 陣列裡南門那筆的 `lat`/`lon` 換成實測值、拿掉 `approx` 標記即可。
...
1. **背景圖 + 校準點對照表（推薦，最簡單）**：匯出你 Figma 地圖的圖片（PNG/SVG），
   ...
   對應到真實世界的位置（可以用建築名稱，我這邊已經有真實 GPS）。我會用跟「南門
   座標估算」（見上方「資料從哪來」）同樣的仿射轉換方法，把整張路網／建築／文華鹿
   位置換算到你的圖片座標系上，疊在你的背景圖上顯示，不用你自己去對每一個路網節點。
...
- **南門座標是估算值，不是實測值**（見上「資料從哪來」）。已經標記 `approx: true`
  並在地圖上用紅色虛線圈提示，如果之後有實測座標請務必替換。
